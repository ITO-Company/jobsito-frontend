...
import { useInternshipListSeeker } from '@/hooks/useInternship'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Calendar, Briefcase } from 'lucide-react'

export function InternshipListSeeker() {
  const navigate = useNavigate()
  const { internships, isLoading, error, fetchMyInternships } = useInternshipListSeeker()
  const [page, setPage] = useState(0)

  const limit = 10

  useEffect(() => {
    fetchMyInternships(limit, page * limit)
  }, [page])
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Pasantías en las que estás registrado</p>
        </div>
        <Button onClick={() => navigate('/jobseeker')} variant="outline">
          Volver
        </Button>
      </div>
      ...
      {isLoading ? (
              ...
              <Card key={internship.id} className="hover:shadow-lg transition-shadow">
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    ...
                    <Button
                      onClick={() => navigate(`/intern/internships/${internship.id}`)}
                      variant="default"
                      size="sm"
                    >
                      Ver Detalle
        ...
        <p className="text-center text-muted-foreground">No tienes pasantías registradas</p>
      )}
    </div>
  )
}
