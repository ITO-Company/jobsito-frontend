...
export const useSignup = (role: UserRole) => {
  ...
  const onSubmit = useCallback(
    async (data: SignupInput) => {
      ...
      try {
        const response =
          role === "job_seeker" ? await authService.jobSeekerSignup(data) : await authService.companySignup(data);
...
export const useSignin = (role: UserRole) => {
  ...
  const onSubmit = useCallback(
    async (data: SigninInput) => {
      ...
      try {
        const response =
          role === "job_seeker" ? await authService.jobSeekerSignin(data) : await authService.companySignin(data);
        ...
        const errorMessage = error.response?.data?.error || "Error en la sesión";
        setError(errorMessage);
        throw error;
      } finally {
        setLoading(false);
